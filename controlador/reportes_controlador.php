...
//        echo "</pre>";exit;
        ...
        for ($x = 0; $x < $n_almacenes; $x++) {
            ...
            for ($i = 0; $i < $datacount; $i++) {
                ...
                $c_descripcion[$contapag] = $c_descripcion[$contapag] . substr(utf8_decode($datos[$i]['PPRODUCTO']), 0, 45) . "\n";
                $c_tipo_producto[$contapag] = $c_tipo_producto[$contapag] . substr(utf8_decode($datos[$i]['SSUBCATEGORIA']), 0, 30) . "\n";
                $c_unidad_medida[$contapag] = $c_unidad_medida[$contapag] . substr($datos[$i]['CCATEGORIA'], 0, 30) . "\n";
                $c_stock[$contapag] = $c_stock[$contapag] . number_format($datos[$i]['STOCK'],0) . "\n";
                $contaobj = $contaobj + 1;
                if ($contaobj == $opp) {
                    $contaobj = 0;
                    $contapag = $contapag + 1;
            ...
            for ($i = $abs; $i <= $contapag; $i++) {
                ...
                $this->_pdf->SetX(28);
                $this->_pdf->Cell(52,6,utf8_decode('Producto'),'BT',0,'L',1);
                $this->_pdf->SetX(80);
                $this->_pdf->Cell(40,6,utf8_decode('Subcategoria'),'BT',0,'L',1);
                $this->_pdf->SetX(120);
                $this->_pdf->Cell(50,6,utf8_decode('Categoria'),'BT',0,'L',1);
                $this->_pdf->SetX(170);
                ...
                $this->_pdf->SetY($Y_table_position);
                $this->_pdf->SetX(28);
                $this->_pdf->MultiCell(52, 5, $c_descripcion[$i], 1);
                $this->_pdf->SetY($Y_table_position);
                $this->_pdf->SetX(80);
                $this->_pdf->MultiCell(40, 5, $c_tipo_producto[$i], 1);
                $this->_pdf->SetY($Y_table_position);
                $this->_pdf->SetX(120);
