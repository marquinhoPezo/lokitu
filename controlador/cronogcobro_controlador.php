...
//            echo '<pre>';print_r($datos_cronogcobro);exit;
            ...
            $producto = '';
            $cantidad_actual = 0;
            if($datos_cronogcobro[0]['MONTO_COBRADO']==0){
                $this->_venta->id_venta = $idventa;
                $datos_venta = $this->_venta->selecciona();
                $this->_detventa->id_venta = $idventa;
                $datos_detventa = $this->_detventa->selecciona();
                
//                echo '<pre>';print_r($datos_detventa);exit;
                for($i=0;$i<count($datos_detventa);$i++){
                    $cantidadub = $datos_detventa[$i]['CANTIDADUB'];
                    $this->_detprodalm->id_almacen = $datos_venta[0]['ID_ALMACEN'];
                    $this->_detprodalm->id_producto = $datos_detventa[$i]['ID_PRODUCTO'];
                    $datos_producto = $this->_detprodalm->selecciona();
//                    echo '<pre>';print_r($datos_producto);exit;
                    $cantidad_actual = $datos_producto[0]['STOCK'];
                    if($cantidadub > $cantidad_actual){
                        $stockactual = $datos_producto[0]['STOCK'];
                        $um = $datos_producto[0]['UUNIDADMEDIDA'];
                        $producto = $datos_detventa[$i]['PPRODUCTO'];
                        $correcto = false;
                    }
                }
                if($correcto){
                    for($i=0;$i<count($datos_detventa);$i++){
                        $this->_detprodalm->id_almacen = $datos_venta[0]['ID_ALMACEN'];
                        $this->_detprodalm->id_producto = $datos_detventa[$i]['ID_PRODUCTO'];
                        $this->_detprodalm->cantidadub=$datos_detventa[$i]['CANTIDADUB'];
                        $this->_detprodalm->id_motivomovimiento = 3;
                        $this->_detprodalm->aumenta=0;
                        $this->_detprodalm->act_stock();
                    }
                }else{
                    echo '<script>alert("El producto '.$producto.' solo cuenta con '.$stockactual.' '.$um.' en stock")</script>';
                    $this->redireccionar('cronogcobro');
                }
            }
...
//            }
//            else{
//                $this->_movimiento->id_formapago = 2;
//            }
