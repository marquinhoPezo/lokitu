        ...
        function buscar(){
            $.post(url+'empleado/buscador','cadena='+$("#buscar").val()+'&filtro='+$("#filtro").val(),function(datos){
                ...
                for(var i=0;i<datos.length;i++){
                    ...
                    HTML += '<td>'+datos[i].SSUCURSAL+'</td>';
                    var editar=url+'empleado/editar/'+datos[i].ID_EMPLEADO; 
                    var eliminar=url+'empleado/eliminar/'+datos[i].ID_EMPLEADO;   
                    HTML += '<td><a style="margin-right:4px" href="#myModal" role="button" data-toggle="modal" onclick="ver(\''+datos[i].ID_EMPLEADO+'\')" class="btn btn-warning btn-minier"><i class="icon-eye-open icon-white"></i> Ver</a>';
